...
palette (green `#16a34a` Â· amber `#d97706` Â· red `#dc2626` Â· gold `#c47f16`),
so the same tone always means the same thing. Intensity deliberately uses its
own non-alarming scale â€” a high intensity is a chosen setting, not a warning.
...
dial: a Laifen handle reports a level of 1â€“10, and 11â€“20 in the high-frequency
...
All icons stay readable in the compact icon-only layout (card width â‰¤ 350 px,
...
an icon appears** â€” [header](#header), [chips and corner
markers](#chips-and-corner-markers), [done badge](#done-badge) â€” because that
...
| :--- | :---: | :---: | :---: | :---: | :---: |
| Battery | â— | â— | â— | â— | â— |
| Pressure | â— | â— | â— | â€” | Wave Pro |
| Intensity | â€” | â€” | â— | â€” | â— |
| Mode | â— | â— | â— settable | â€” | â— settable |
| Score | â€” | â€” | â€” | â— | â€” |
| Brush head | â€” | â€” | â— sub-device | â— % left | â€” |
| Head type | â€” | â€” | â— sub-device | â€” | â€” |
| Sector | reported | reported | derived | from time | from time |
| ESP bridge icon | â€” | â€” | â— | â€” | â€” |
| Charger icon | â€” | â— | â€” | â€” | â€” |
| Display face | â€” | â— | â€” | â€” | â€” |
...
it. And a sector is only *reported* by the Oral-B integrations â€” Sonicare
...
the way into the device page. These are **transport** states â€” they say how the
...
creates it for no other transport â€” which is what makes its third state honest:
...
One reading each â€” battery, pressure, intensity, mode, score, brush head, head
...
just finished, latched at its end and cleared at the next start â€” not a live
...
describes the session that just ended â€” which is what the badge is for.
`session-state.js` holds it as `completedScore`.

Both are drawn at 34 px rather than the chips' 24 px â€” the star-eyes face
...
| â€” | *reserved* | `mdi:emoticon-neutral-outline` | amber `#d97706` |
| â€” | *reserved* | `mdi:emoticon-sad-outline` | red `#dc2626` |
| `special_2` â€¦ `special_9` | undecoded | `mdi:help-circle-outline` + raw value | muted `#9ca3af` |
...
but never alongside a face â€” one handle, one verdict.
...
a rank won â€” there is nobody to beat. A podium would also read as a bar chart
...
`special_2` â€¦ `special_9` are still open.
