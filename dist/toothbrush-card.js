...
class $930552a63f9e9686$export$e2f41388bb2b94a0 extends (0, $528e4332d1e3099e$export$3f2f9f5909897157) {
    ...
    _recapFromLastSession(hass, config, entityIds, routineFromEntity, notBefore = 0) {
        ...
        if (notBefore) {
            ...
            if (mark !== null && mark !== undefined && recordId !== null) {
                ...
                if (recordId < mark) {
                    ...
                    if (endedAt < notBefore - 60000) return false;
                }
            } else if (endedAt < notBefore - 60000) return false;
        }
        ...
        const target = Number(config.routine_length) || Number(attrs.routine_length_seconds ?? attrs.target_duration_seconds) || routineFromEntity || (entityIds.routine_length || entityIds.routine_length_number ? 0 : (0, $b973a26f761c9c78$export$918b2e620e4fca36));
    ...
    render() {
        ...
        if (latch.needsRecap) {
            ...
            const fromRecord = config.device_recap !== false && this._recapFromLastSession(hass, config, entityIds, target);
            if (!fromRecord && config.history_recap !== false) this._maybeLoadRecapFromHistory(hass, config, entityIds, target);
        } else if (this._completed && this._completedSource !== 'device' && config.device_recap !== false && entityIds.last_session) // A session the card watched end, on a handle that files a record
        ...
        const ownVerdict = showVerdict && this._completedSource === 'device' && !this._completedFace && !Number.isFinite(parseFloat(this._completedScore)) && this._completedDuration > 0 && Number.isFinite(this._completedPressure) ? (()=>{
